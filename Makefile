...
EXT_PATH      := $(CURDIR)/target/debug/libinfer.$(EXT_SUFFIX)

test: build
	@test -f "$(EXT_PATH)" || { echo "missing $(EXT_PATH) — run 'make build'"; exit 1; }
	$(PHP) -d extension=$(EXT_PATH) \
		-r 'if (!extension_loaded("infer")) { fwrite(STDERR, "infer not loaded\n"); exit(1); }'
...
stubs: check-cargo-php build
	$(CARGO) php stubs --stubs stubs/infer.stubs.php
